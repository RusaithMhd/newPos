    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const [suppliersRes, storesRes, productsRes] = await Promise.all([
          axios.get(`/api/suppliers?_t=${timestamp}`, config),
          axios.get(`/api/store-locations?_t=${timestamp}`, config),
          axios.get(`/api/products?_t=${timestamp}`, config),
        ]);
        ...
        if (productsData.length === 0) {
          setErrors((prev) => ({
            ...prev,
            products:
              "No products available. Please add products in the system.",
          }));
        }
      } catch (error) {
        const errorMsg = error.response?.data?.message || error.message;
        setErrors({ fetch: `Error fetching data: ${errorMsg}` });
